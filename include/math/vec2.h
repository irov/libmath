...
namespace mt
{
	struct vec2f
	{
		...
		MT_METHOD_INLINE vec2f(const vec2f & _v);
		MT_METHOD_INLINE explicit vec2f( const float * _v );

		MT_METHOD_INLINE vec2f & operator = (const vec2f & _rhs);
		...
		MT_METHOD_INLINE vec2f& operator+=(const vec2f & _rhs);
		MT_METHOD_INLINE vec2f& operator-=(const vec2f &	_rhs);
		MT_METHOD_INLINE vec2f& operator/=(const float _rhs);
		MT_METHOD_INLINE vec2f& operator*=(const float _rhs);
		MT_METHOD_INLINE vec2f& operator/=(const vec2f & _rhs);
		MT_METHOD_INLINE vec2f& operator*=(const vec2f & _rhs);
	...
	MT_FUNCTION_INLINE void norm_safe_v2(vec2f& out, const vec2f& _rhs, float err = 0.00001f);
	...
	MT_FUNCTION_INLINE void perp_v2(vec2f& out, const vec2f& in);
	MT_FUNCTION_INLINE vec2f perp(const vec2f&	in);

	MT_FUNCTION_INLINE void perp_left_v2(vec2f& out, const vec2f& in);
	MT_FUNCTION_INLINE vec2f perp_left(const vec2f&	in);
